...
DEVICE_PROP Device_Property =
  {
    ...
    HID_Comm_Status_Out,
    HID_Comm_Data_Setup,
    HID_Comm_NoData_Setup,
    HID_Comm_Get_Interface_Setting,
    HID_Comm_GetDeviceDescriptor,
    HID_Comm_GetConfigDescriptor,
    HID_Comm_GetStringDescriptor,
    0,
    0x40 /*MAX PACKET SIZE*/
  };
USER_STANDARD_REQUESTS User_Standard_Requests =
  {
    HID_Comm_GetConfiguration,
    HID_Comm_SetConfiguration,
    HID_Comm_GetInterface,
    HID_Comm_SetInterface,
    HID_Comm_GetStatus,
    HID_Comm_ClearFeature,
    HID_Comm_SetEndPointFeature,
    HID_Comm_SetDeviceFeature,
    HID_Comm_SetDeviceAddress
  };

ONE_DESCRIPTOR Device_Descriptor =
  {
    (uint8_t*)HID_Comm_DeviceDescriptor,
    HID_Comm_SIZ_DEVICE_DESC
  };

ONE_DESCRIPTOR Config_Descriptor =
  {
    (uint8_t*)HID_Comm_ConfigDescriptor,
    HID_Comm_SIZ_CONFIG_DESC
  };

ONE_DESCRIPTOR HID_Comm_Report_Descriptor =
  {
    (uint8_t *)HID_Comm_ReportDescriptor,
    HID_Comm_SIZ_REPORT_DESC
  };

ONE_DESCRIPTOR Mouse_Hid_Descriptor =
  {
    (uint8_t*)HID_Comm_ConfigDescriptor + HID_Comm_OFF_HID_DESC,
    HID_Comm_SIZ_HID_DESC
  };

ONE_DESCRIPTOR String_Descriptor[4] =
  {
    {(uint8_t*)HID_Comm_StringLangID, HID_Comm_SIZ_STRING_LANGID},
    {(uint8_t*)HID_Comm_StringVendor, HID_Comm_SIZ_STRING_VENDOR},
    {(uint8_t*)HID_Comm_StringProduct, HID_Comm_SIZ_STRING_PRODUCT},
...
void Get_SerialNum(void)
{
  ...
  if (Device_Serial0 != 0)
  {
    IntToUnicode (Device_Serial0, &HID_Comm_StringSerial[2] , 8);
    IntToUnicode (Device_Serial1, &HID_Comm_StringSerial[18], 4);
  }
}
...
{
  /* Set HID_Comm_DEVICE as not configured */
  pInformation->Current_Configuration = 0;
  pInformation->Current_Interface = 0;/*the default Interface*/

  /* Current Feature initialization */
  pInformation->Current_Feature = HID_Comm_ConfigDescriptor[7];
...
/**
  ...
  */
RESULT HID_Comm_Data_Setup(uint8_t RequestNo)
{
      ...
      && (pInformation->USBwIndex0 == 0))
  {
    ...
    {
      CopyRoutine = HID_Comm_GetReportDescriptor;
    }
    else if (pInformation->USBwValue1 == HID_DESCRIPTOR_TYPE)
    {
      CopyRoutine = HID_Comm_GetHIDDescriptor;
    }
  ...
  {
    CopyRoutine = HID_Comm_GetProtocolValue;
  }
...
/**
  ...
  * @retval USB_UNSUPPORT or USB_SUCCESS.
  */
RESULT HID_Comm_NoData_Setup(uint8_t RequestNo)
{
  if ((Type_Recipient == (CLASS_REQUEST | INTERFACE_RECIPIENT))
      && (RequestNo == SET_PROTOCOL))
  {
    return HID_Comm_SetProtocol();
  }
...
/**
  ...
  */
uint8_t *HID_Comm_GetDeviceDescriptor(uint16_t Length)
{
...
/**
  ...
  */
uint8_t *HID_Comm_GetConfigDescriptor(uint16_t Length)
{
...
/**
  ...
  */
uint8_t *HID_Comm_GetStringDescriptor(uint16_t Length)
{
...
{
  return Standard_GetDescriptorData(Length, &HID_Comm_Report_Descriptor);
}
