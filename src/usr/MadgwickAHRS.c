...
void MadgwickAHRSupdate(Senseordata_Type*Sensondata) {
	...
	if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
		MadgwickAHRSupdateIMU(Sensondata);
		return;
	}
...
float inters0,inters1,inters2,inters3;	 
float GryErr0,GryErr1,GryErr2,GryErr3;
float s0, s1, s2, s3;
float grecipNorm;
void GetGryErr(float* gryerr)
{
    //gryerr[0] = GryErr0*57.3f;
    gryerr[0] =    GryErr1*57.3f/grecipNorm;
    gryerr[1] =   -GryErr3*57.3f/grecipNorm;
    gryerr[2] =    GryErr2*57.3f/grecipNorm;
}
void GetGryDynOffset(float* gryerr)
{
    //gryerr[0] = GryErr0*57.3f;
    gryerr[0] =   inters1*57.3f;
    gryerr[1] =  -inters3*57.3f;
    gryerr[2] =   inters2*57.3f;
}

void  SetGryInterZero(uint8_t index,float trust)
{
    inters0 = inters0*trust;
    if(index == 0)inters1 = inters1*trust;
    if(index == 1)inters3 = inters3*trust;
    if(index == 2)inters2 = inters2*trust;
    
}
float qDotSum = 0;

float IntLimite(float v,float m)
{
	if(v>=m)return m;
	if(v<=-m)return -m;
	return v;
...
uint8_t accflat = 0;
	...
	// Gradient decent algorithm corrective step
		...
		s2 *= recipNorm;
		s3 *= recipNorm;
		grecipNorm = recipNorm;//应该加入限幅和误差和低通滤波
		GryErr0 = 2* (q0*s0+q1*s1+q2*s2+q3*s3);
		GryErr1 = 2* (q0*s1-q1*s0-q2*s3+q3*s2);
		GryErr2 = 2* (q0*s2+q1*s3-q2*s0-q3*s1);
		GryErr3 = 2* (q0*s3-q1*s2+q2*s1-q3*s0);            
    
		
		
    if(!(Mgdgki == 0.0f)){
        inters0+=(GryErr0)*Mgdgki*deltat;
				inters0 = IntLimite(inters0,0.05f);
        inters1+=(GryErr1)*Mgdgki*deltat;
				inters1 = IntLimite(inters1,0.05f);
        inters2+=(GryErr2)*Mgdgki*deltat;
				inters2 = IntLimite(inters2,0.05f);
        inters3+=(GryErr3)*Mgdgki*deltat;  
				inters3 = IntLimite(inters3,0.05f);  
    }
    gx -= inters1;
    gy -= inters2;
    gz -= inters3;
  ...
  if(accflat){
      ...
      qDot1-= 0.5f*q0*inters0;
      qDot2-= 0.5f*q1*inters0;
      qDot3-= 0.5f*q2*inters0;
      qDot4-= 0.5f*q3*inters0;
